...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Checkout-Type', 'X-Client-Timestamp']
}));
...
app.post(['/api/checkout', '/api/create-checkout-session'], async (req, res) => {
  try {
...
// Start the server
app.listen(PORT, () => {
  console.log(`[${new Date().toISOString()}] Test checkout server running on port ${PORT}`);
  console.log(`[${new Date().toISOString()}] API URL: http://localhost:${PORT}/api`);
  console.log(`[${new Date().toISOString()}] Using Stripe Secret Key: ${stripeSecretKey ? '✓ Configured' : '✗ Missing'}`);
  console.log(`[${new Date().toISOString()}] Ready to process checkout requests`);
}); 